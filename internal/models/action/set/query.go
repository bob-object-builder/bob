...
func NewQuery(block lexer.Block) Set {
	get := Set{Filters: []lexer.Instruction{}}
	...
	for _, child := range block.Children() {
		...
		case lexer.Block:
			if v.ActionIs(lexer.LeftJoin, lexer.LeftJoinAlias) {
				...
				subQuery := NewQuery(v)
				get.Values.Set(subQuery.Alias, subQuery)
			}
		}
	}
